...
export const ResetPasswordCode = ({
  ...
  <Tailwind>
    <Html>
      <Head />
      <Preview>{code} is your password reset code</Preview>
      <Body className="flex min-h-screen items-center justify-center bg-white text-center font-sans">
        <Container className="mx-auto w-full max-w-md rounded-lg border border-black bg-white p-6">
          ...
          <Text className="mt-2 text-sm text-black">
            Use this code to reset your password or press the reset password in
            button. This code will expire in {expiryDate}{" "}
            {expiryDate > 1 ? expiryDateUnit + "s" : expiryDateUnit}.
          </Text>
          <Section className="mx-auto mt-6 max-w-sm rounded-md bg-gray-100 py-4">
            <Heading className="">{code}</Heading>
          </Section>
          <Section className="mt-6">
            <Button
              href={`${getStoreBaseUrl()}/reset-password?indentifier=${email}?code=${code}`}
