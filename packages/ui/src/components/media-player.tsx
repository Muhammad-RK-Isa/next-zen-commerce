...
import { useComposedRefs } from "@nzc/ui/lib/composition"
...
function createStore(listeners: Set<() => void>, initialState: MediaState) {
  ...
  function dispatch(action: StoreAction) {
    // biome-ignore  lint/style/useDefaultSwitchClause: default case has no use
        ...
        <TooltipTrigger asChild>
          ...
          >
            ...
            <SliderPrimitive.Thumb
              ...
              className="bg-primary ring-ring/50 focus-visible:outline-hidden relative z-10 block size-2.5 shrink-0 rounded-full shadow-sm transition-[color,box-shadow] hover:ring-4 focus-visible:ring-4 disabled:pointer-events-none disabled:opacity-50"
            />
          </SliderPrimitive.Root>
        </TooltipTrigger>
        {duration > 0 && (
    ...
    >
      ...
      >
        ...
        <SliderPrimitive.Thumb
          ...
          className="bg-primary ring-ring/50 focus-visible:outline-hidden block size-2.5 shrink-0 rounded-full shadow-sm transition-[color,box-shadow] hover:ring-4 focus-visible:ring-4 disabled:pointer-events-none disabled:opacity-50"
        />
      </SliderPrimitive.Root>
    </div>
  )
    ...
    >
      <MediaPlayerTooltip tooltip="Playback speed" shortcut={["<", ">"]}>
        <SelectTrigger
          ...
          ref={forwardedRef}
          className={cn(
            "aria-[expanded=true]:bg-accent/50 dark:aria-[expanded=true]:bg-accent/50 dark:hover:bg-accent/50 h-8 w-16 justify-center border-none dark:bg-transparent [&[data-size]]:h-8 [&_svg]:hidden",
            className
          )}
        >
        ...
        {Array.isArray(shortcut) ? (
          <div className="flex items-center gap-1">
            {shortcut.map((shortcutKey) => (
              <kbd
                key={shortcutKey}
                className="bg-secondary text-foreground shadow-xs select-none rounded border px-1.5 py-0.5 font-mono text-[0.7rem]"
              >
          ...
          <kbd
            key={shortcut}
            className="bg-secondary text-foreground shadow-xs select-none rounded border px-1.5 py-px font-mono text-[0.7rem]"
          >
