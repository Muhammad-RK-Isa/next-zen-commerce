...
import { Calendar } from "@nzc/ui/components/calendar"
import {
  Command,
  CommandEmpty,
  CommandGroup,
...
import { DataTableRangeFilter } from "@nzc/ui/components/data-table/data-table-range-filter"
import {
  Faceted,
  FacetedBadgeList,
  FacetedContent,
  ...
  FacetedList,
  FacetedTrigger,
} from "@nzc/ui/components/faceted"
...
import {
  ...
  SortableItemHandle,
  SortableOverlay,
} from "@nzc/ui/components/sortable"
import { dataTableConfig } from "@nzc/ui/config/data-table"
import {
  getDefaultFilterOperator,
  getFilterOperators,
} from "@nzc/ui/lib/data-table"
import { getFiltersStateParser } from "@nzc/ui/lib/parsers"
import { cn, generateId } from "@nzc/ui/lib/utils"
import type {
...
export function DataTableFilterList<TData>({
  ...
  const debouncedSetFilters = useDebounceCallback(setFilters, debounceMs)
    ...
    case "dateRange": {
      ...
      const displayValue =
        filter.operator === "isBetween" && dateValue.length === 2
          ? `${formatDate(new Date(Number(dateValue[0])), "MMMM d, yyyy")} - ${formatDate(
              new Date(Number(dateValue[1])),
              "MMMM d, yyyy"
            )}`
          : dateValue[0]
            ? formatDate(new Date(Number(dateValue[0])), "MMMM d, yyyy")
            : "Pick a date"
