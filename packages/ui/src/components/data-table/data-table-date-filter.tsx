...
export function DataTableDateFilter<TData>({
  ...
  const formatDateRange = React.useCallback((range: DateRange) => {
    ...
    if (range.from && range.to) {
      return `${formatDate(range.from, "MMMM d, yyyy")} - ${formatDate(range.to, "MMMM d, yyyy")}`
    }
    const dateRange = range.from ?? range.to
    if (!dateRange) {
      return ""
    }
    return formatDate(dateRange, "MMMM d, yyyy")
  }, [])
  ...
  const label = React.useMemo(() => {
    ...
    const dateText = hasSelectedDate
      ? // biome-ignore  lint/style/noNonNullAssertion: The first is always guaranted to be not-null
        formatDate(selectedDates[0]!, "MMMM d, yyyy")
      : "Select date"
